...
import { PrismaClient, UserRole, SubscriptionStatus, PropertyType, PropertyStatus, ContractStatus, InvoiceType, InvoiceStatus } from '@prisma/client';
import * as bcrypt from 'bcryptjs';
...
async function main() {
  ...
  // 1. NETTOYAGE (Optionnel : à décommenter si tu veux tout effacer à chaque seed)
  // await prisma.paymentTransaction.deleteMany();
  // await prisma.invoice.deleteMany();
  // await prisma.contract.deleteMany();
  // await prisma.tenant.deleteMany();
  ...
  // Hash du mot de passe pour tous les utilisateurs de test
  const passwordHash = await bcrypt.hash('password123', 10);

  // 3. ORGANISATION 1 : Agence Immobilière (Pro)
  const org1 = await prisma.organization.create({
    data: {
      name: 'Agence Immotogo',
      email: 'contact@immotogo.com',
      phone: '+22890000001',
      planId: planPro.id,
  ...
  // Création du profil Owner pour Admin1 (il est proprio de ses propres biens potentiellement)
  await prisma.owner.create({
    data: { userId: admin1.id }
  });

  // Locataires de l'Agence
  const tenant1User = await prisma.user.create({
    data: {
      email: 'yaw.tenant@gmail.com',
      password: passwordHash,
      firstName: 'Yaw',
      lastName: 'Doe',
  ...
  await prisma.tenant.create({
    data: { userId: tenant1User.id, oldAddress: 'Lomé Ancien Quartier' }
  });

  const tenant2User = await prisma.user.create({
    data: {
      email: 'afia.tenant@gmail.com',
      password: passwordHash,
      firstName: 'Afia',
      lastName: 'Mensah',
      role: UserRole.TENANT,
  ...
  await prisma.tenant.create({
    data: { userId: tenant2User.id }
  });

  console.log('Org 1 and users created');

  // 4. ORGANISATION 2 : Propriétaire Solo (Basic)
  const org2 = await prisma.organization.create({
    data: {
      name: 'Patrimoine de M. Kofi',
      email: 'kofi.perso@gmail.com',
      phone: '+22890000002',
      planId: planBasic.id,
      subscriptionStatus: SubscriptionStatus.TRIAL,
  ...
  await prisma.tenant.create({
    data: { userId: tenant3User.id }
  });

  console.log('Org 2 and users created');

  // 5. Création d'un Contrat Test (lié à l'Org1)
  const owner1Profile = await prisma.owner.findFirst({ where: { userId: admin1.id } });
  const tenant1Profile = await prisma.tenant.findFirst({ where: { userId: tenant1User.id } });
